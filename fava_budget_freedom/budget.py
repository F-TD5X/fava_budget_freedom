...
from .utils import get_budget_effective_date, parse_amount
...
class BudgetParser:
    ...
    def parse_budget_definitions(self, end_date):
        ...
        Args:
            end_date: The inclusive cutoff date for budget definitions.

        Returns:
            A dictionary mapping patterns to a list of budget definitions sorted by
            effective date.
        """
        budget_definitions = []
            ...
            if isinstance(entry, data.Custom) and entry.type == "budget":
                ...
                if len(entry.values) >= 3:
                    ...
                    budget_definitions.append(
                        {
                            'date': entry.date,
                            'effective_date': get_budget_effective_date(
                                entry.date, period
                            ),
                            'pattern': pattern,
                            'amount': amount,
        ...
        for budget in budget_definitions:
            budgets_by_pattern[budget['pattern']].append(budget)

        for pattern in budgets_by_pattern:
            budgets_by_pattern[pattern].sort(
                key=lambda budget: (budget['effective_date'], budget['date'])
            )
