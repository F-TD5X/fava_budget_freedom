...
- **预算滚存 (Rollover)**：支持月度预算的滚存功能，上月未用完的额度自动累积到下月，超支则扣减下月额度，并在每个自然年开始时重置。
- **持续生效的预算规则**：预算指令会一直生效，直到同一模式下出现更新的指令覆盖它。
...
![Budget Freedom 截图](assets/screenshot.png)
...
- **rollover**: (可选) 仅适用于 `monthly`，开启后支持当前自然年内的预算累积。
